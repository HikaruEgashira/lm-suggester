...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:   "reviewdog-converter",
		Short: "Convert LLM suggestions to reviewdog JSON format",
		Long: `reviewdog-converter transforms suggestions from LLMs and other external tools
into reviewdog-compatible JSON format for code review automation.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cliInput.SourceName == "" {
				cliInput.SourceName = "reviewdog-converter"
			}
	...
	versionCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("reviewdog-converter %s\n", version)
			fmt.Printf("  commit: %s\n", commit)
